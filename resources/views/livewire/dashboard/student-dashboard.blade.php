...
new class extends Component {
    ...
    {
        return view('components.skeleton.student-dashboard')->render();
    }
    ...
    public function with(): array
    {
        ...
        // Handle jika profil belum lengkap (bisa juga dihandle di middleware)
        if (!$student) {
            return [
                'totalSubmissions' => 0,
                'pendingCount' => 0,
                ...
                'rejectedCount' => 0,
                'verifiedCount' => 0,
                'recentSubmissions' => collect([]) // Collection kosong
            ]; 
        }

        // Query Base
        $queryBase = Submission::where(function ($query) use ($student) {
            $query->where('representative_nim', $student->nim)
                ->orWhereHas('members', fn($q) => $q->where('student_nim', $student->nim));
        });

        // Ambil Stats (Single Query Optimized)
        ...
        // Ambil Recent Data
        $recentSubmissions = (clone $queryBase)
